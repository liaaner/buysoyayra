    ...
    <div className="flex-1 flex flex-col w-full px-8 sm:max-w-md justify-center gap-2 mx-auto mt-24 pb-24">
      ...
      <form className="animate-in flex-1 flex flex-col w-full justify-center gap-4 text-foreground">
        ...
        <input
          ...
          required
        />
        <button formAction={signup} className="bg-primary text-primary-foreground rounded-md px-4 py-3 mt-4 hover:opacity-90 font-medium">
          Sign Up
        </button>
        <p className="text-sm text-center text-muted-foreground mt-4">
          Already have an account?{" "}
          <Link href="/login" className="underline text-foreground cursor-pointer">
            Sign in
          </Link>
        </p>
      </form>
